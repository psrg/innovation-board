...
//            CONFIGURE_RTS(control_signal_bitmap.CARRIER_CONTROL);
            //------------------------------------------------------------------            
                ...
                {
                  #ifdef USB_CDC_FLOAT_DTR
                    mInitDTRPin();

                    // We can't simply set the line active, since the pin
                    // was disabled previously (and therefore, has an undefined
                    // voltage). If the pin's voltage floated down to GND,
                    // then turning on the output and driving the pin to GND
                    // will have no effect, since the pin is capacitively-
                    // coupled to the reset line.
                    //
                    // TODO: In future hardware versions, we should directly-
                    // couple the reset pin to the "DTR" pin.
                    
                    UART_DTR = (USB_CDC_DTR_ACTIVE_LEVEL ^ 1);
                    int i;
                    for(i=0; i< 1000; i++); // fair for a few moments
                    #endif
